...
py.gmsh4mrst.background_grid_2D(...
    ...
    FC_mesh_sampling = params.FCMeshSampling, ...
...
% Remove points within wellPts
if isempty(pGs)
    distanceFromWellPts = cGs;
else
    distanceFromWellPts = min(cGs, pGs);
end
Pts = removeConflictPoints(G.nodes.coords, wellPts, distanceFromWellPts);
...
if params.sufFCCond
	Pts = surfaceSufCond2D(Pts,F);
else
	Pts = removeConflictPoints(Pts,F.f.pts, F.f.Gs);
end
